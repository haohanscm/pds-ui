    ...
    <div class="goods-item" :style="itemStyle">
        ...
        <div class="goods-info-wrap">
            ...
            <div class="goods-model">
                {{goods.goodsModel > 0 ? '多规格' : ''}}
            </div>
            <div class="goods-price" v-if="inCart">
                <div v-if="goods.goodsPrice">
                    <span class="fs14"><span>¥ </span>{{goods.goodsPrice}}</span>
                    <span class="fs14 txt-del txt-999"><span>¥ </span>{{goods.marketPrice}}</span>
                </div>
                <div class="fs14" v-else><span>¥ </span>{{goods.marketPrice}}</div>
                <div class="fs16 txt-red"><span class="fs14">¥ </span>{{computedPrice}}</div>
            </div>
            <div class="goods-price" v-if="!inCart">
                <div class="fs16 txt-red"><span class="fs14">¥ </span>{{goods.marketPrice}}</div>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        computedPrice: function() {
            let price = this.goods.goodsPrice || this.goods.marketPrice
