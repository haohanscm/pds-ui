    ...
    export default {
        data: function(){
            return {
                ...
                period: "0",
                date:{
                    startTime: this.$utils.getDay(-7),
                    endTime: this.$utils.getDay(0)
                }
            }
        },
        mounted(){
        ...
        methods:{
            ...
            changeCond(){
                if(this.period === "0"){
                    this.date.startTime = this.$utils.getDay(-7)
                    this.date.endTime = this.$utils.getDay(0)
                }else if(this.period === "1"){
                    this.date.startTime = this.$utils.getDay(-30)
                    this.date.endTime = this.$utils.getDay(0)
                }
                this.statisCurve()
            },
            statisCurve(){
                this.request.statisCurve({...this.date}).then( res => {
                    if( res.code === 200){
                        ...
                        let arr3 = []
                        arr.forEach(ele => {
                            arr1.push(ele.orderDate)
                            arr2.push(ele.saleAmount)
